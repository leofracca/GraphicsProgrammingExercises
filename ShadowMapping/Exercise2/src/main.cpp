/*
* Exercise 2 - Shadow Mapping, with an animated model
* 
...
void renderScene(Shader& shader, const Object cubes[3], Model &modelObj, Animator &animator, const Object &modelProperties)
{
    // floor
    ...
    for (int i = 0; i < transforms.size(); ++i)
        shader.setMatrix4f("u_finalBonesMatrices[" + std::to_string(i) + "]", transforms[i]);

    // render the loaded model
    model = glm::mat4(1.0f);
    model = glm::translate(model, modelProperties.position/*glm::vec3(0.0f, -0.5f, 0.0f)*/);
    model = glm::rotate(model, modelProperties.rotationAngle, modelProperties.rotation);
    model = glm::scale(model, modelProperties.scale);
    shader.setMatrix4f("u_model", model);
    modelObj.Draw(shader);
}

void renderScene(Shader& shader, const Object cubes[3])
{
    // floor
    glm::mat4 model = glm::mat4(1.0f);
    shader.setMatrix4f("u_model", model);
...
int main()
{
    ...
    Shader modelShader("res/shaders/anim_model.vs", "res/shaders/anim_model.fs");

    // load models
    // -----------
    //std::string modelName = "res/objects/vampire/dancing_vampire.dae";
    std::string modelName = "res/objects/zombie/dancing_zombie.dae";
    Model ourModel(modelName);
    Animation danceAnimation(modelName, &ourModel);
    ...
    PlaneTexture woodTexture("res/textures/wood.png", GL_CLAMP_TO_EDGE, true);
    ...
    PlaneTexture depthMap(SHADOW_WIDTH, SHADOW_HEIGHT);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        woodTexture.bind();
        renderScene(simpleDepthShader, cubes, ourModel, animator, modelProperties);
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        modelShader.setMatrix4f("u_projection", projection);
        modelShader.setMatrix4f("u_view", view);

        auto transforms = animator.GetFinalBoneMatrices();
        for (int i = 0; i < transforms.size(); ++i)
            modelShader.setMatrix4f("u_finalBonesMatrices[" + std::to_string(i) + "]", transforms[i]);

        // Render the loaded model
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, modelProperties.position);
        model = glm::rotate(model, modelProperties.rotationAngle, modelProperties.rotation);
        model = glm::scale(model, modelProperties.scale);
        modelShader.setMatrix4f("u_model", model);
        ...
        // Control objects with Dear ImGui
        {
            ...
            if (ImGui::BeginTabBar("CubesProperties", ImGuiTabBarFlags_None))
            {
                ...
                {
                    ImGui::SliderFloat3("Position", &modelProperties.position.x, -10.0f, 10.0f);
                    ImGui::SliderFloat3("Scale", &modelProperties.scale.x, 0.1f, 10.0f);
                    ImGui::SliderFloat3("RotationAxis", &modelProperties.rotation.x, 0.0f, 1.0f);
                    ImGui::SliderAngle("RotationAngle", &modelProperties.rotationAngle); // The slider values go from -360 to 360 (degrees), but the value is saved in radians
                    ImGui::EndTabItem();
                }
                ImGui::EndTabBar();
            }
