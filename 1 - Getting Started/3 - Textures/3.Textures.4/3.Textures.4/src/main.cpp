...
const std::string TITLE = "Textures4";

void getMixValue(GLFWwindow* window, float& currentMixValue)
{
    // Change textures mix value with arrow keys
    /*constexpr float increment = 0.005f;

    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)
    {
        currentMixValue += increment;
        if (currentMixValue >= 1.0f)
            currentMixValue = 1.0f;
    }
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
    {
        currentMixValue -= increment;
        if (currentMixValue <= 0.0f)
            currentMixValue = 0.0f;
    }*/

    // Change textures mix value with time
    float timeValue = glfwGetTime();
    currentMixValue = abs(sin(timeValue) / 2.0f + 0.5f);
}

void processInput(GLFWwindow* window, float &currentMixValue)
{
    if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
        glfwSetWindowShouldClose(window, true);

    getMixValue(window, currentMixValue);
}
...
int main()
{
    ...
    float textureMixValue = 0.0f;
    while (!glfwWindowShouldClose(window))
    {
        processInput(window, textureMixValue);
        ...
        shader.setFloat("u_mixValue", textureMixValue);
