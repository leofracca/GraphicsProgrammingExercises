...
* If you did things right the lighting should look a bit off (especially the specular highlights) with the cube object.
* Try to reason why it looks so weird
* 
* Answer: Since we are now calculating the light reflection in the vertices (in the vertex shader)
* and not in the single fragment (fragment shader), the result is that the color calculated in the vertices
* gets interpolated to get the color inside the triangles.
* This causes an unnatural effect, since the light highlights the two separated triangles.
*/
...
const std::string TITLE = "BasicLighting3";
