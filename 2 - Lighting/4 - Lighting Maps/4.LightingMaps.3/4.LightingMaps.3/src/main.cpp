...
* stores emission values per fragment. Emission values are colors an object
* may emit as if it contains a light source itself; this way an object can
* glow regardless of the light conditions. Emission maps are often what
* you see when objects in a game glow (like eyes of a robot, or light strips on a container)
*/
...
const std::string TITLE = "LightingMaps3";
...
int main()
{
    ...
    Texture emissionMap("res/textures/matrix.jpg", GL_REPEAT); // We use GL_REPEAT so we can make "the matrix" slide (i.e. make a cool effect ;))
    lightingShader.use();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        lightingShader.setVec3("u_light.ambient", 0.2f, 0.2f, 0.2f);
        lightingShader.setVec3("u_light.diffuse", 0.5f, 0.5f, 0.5f); // Darken diffuse light a bit
        lightingShader.setVec3("u_light.specular", 1.0f, 1.0f, 1.0f);
        // Set the camera position
        lightingShader.setVec3("u_viewPos", camera.GetPosition());

        // Used just for funny effect on the matrix emission map (sliding and fading)
        lightingShader.setFloat("u_time", glfwGetTime());
